...
Madgwick::Madgwick(void) {
    sampleFreq = 100.0f; // Default
    beta = 0.1f;         // Default
    q0 = 1.0f;
    q1 = 0.0f;
    q2 = 0.0f;
...
float Madgwick::invSqrt(float x) {
    return 1.0f / sqrtf(x); // Standard sqrt is often fast enough on modern MCUs like nRF52
    // If needed, we can use the 0x5f3759df magic number method, 
    // but on nRF52 FPU, standard sqrt is hardware accelerated.
}
...
// Minimal implementation of update with Mag ignored for now (since we don't use it yet)
